...
@cocotb.test()
async def test_joy_diverge_persist_rom_matches_pyboy_signature(dut):
    driver = cpu_dut(dut)
    await driver.reset(ResetProfile.SkipBoot)
    await assert_rom_matches_pyboy_signature(driver, rom_id="JOY_DIVERGE_PERSIST", max_mcycles=20000)
