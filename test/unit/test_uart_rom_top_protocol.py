# top = board::icebreaker_uart_rom_top::icebreaker_uart_rom_protocol_test_top
...
@cocotb.test()
async def test_happy_path_uploads_256_byte_payload_and_deasserts_reset(dut):
    sim = await reset_dut(dut)
    payload = bytes((index * 17 + 3) & 0xFF for index in range(256))
