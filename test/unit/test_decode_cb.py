# top = cpu::decode_test_top::decode_cb_test_top
...
from tools.sm83_decode_reference import projection_for_cb_opcode
...
@cocotb.test()
async def test_cb_decoder_matches_projection_for_all_prefixed_opcodes(dut):
    for opcode in range(0x100):
        snapshot = await sample(dut, opcode=opcode)
        assert snapshot == projection_for_cb_opcode(opcode), (opcode, snapshot, projection_for_cb_opcode(opcode))
