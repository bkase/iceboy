...
DRY_RUN=0
SKIP_BUILD=0
OUT_DIR="${ICEBOY_ROOT}/build/hw_probes/framebuffer"
VERIFY_SCRIPT="${ICEBOY_ROOT}/tools/verify_icebreaker_variant.sh"
TOP_LABEL="video::framebuffer_probe_top::framebuffer_probe_top"
TOP_MODULE="framebuffer_probe_top"
BOARD_TOP="${ICEBOY_ROOT}/src/video/framebuffer_probe_top.spade"
NETLIST_JSON=""
STAT_FILE=""
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help|-h)
            cat <<'EOF'
usage: tools/run_framebuffer_synth_probe.sh [--dry-run] [--skip-build] [--out-dir <dir>]
EOF
            exit 0
            ;;
        *)
...
iceboy_require_file "${NETLIST_JSON}" "synthesized framebuffer netlist"
...
echo "Framebuffer probe resources: SB_SPRAM256KA=${SPRAM_COUNT} SB_LUT4=${LUT4_COUNT} SB_DFF=${DFF_COUNT} SB_RAM40_4K=${EBR_COUNT}"

if (( SPRAM_COUNT != 1 )); then
    iceboy_die "expected framebuffer probe to use exactly 1 SB_SPRAM256KA block, got ${SPRAM_COUNT}"
fi
if (( LUT4_COUNT >= 200 )); then
    iceboy_die "expected framebuffer probe to stay under 200 SB_LUT4 cells, got ${LUT4_COUNT}"
fi
if (( DFF_COUNT >= 50 )); then
    iceboy_die "expected framebuffer probe to stay under 50 SB_DFF cells, got ${DFF_COUNT}"
fi
if (( EBR_COUNT != 0 )); then
    iceboy_die "expected framebuffer probe to avoid EBR, got SB_RAM40_4K=${EBR_COUNT}"
fi
