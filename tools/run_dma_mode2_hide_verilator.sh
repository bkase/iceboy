...
#
# This ROM checkpoints before the first visible sprite frame. PyBoy's
# `settle_rendered_frames=2` lands on the hidden frame, but the native runner's
# completed-frame counter increments at frame start, so it needs one extra
# completed frame to reach the same rendered image.
...
exec "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/run_ppu_wave_c_verilator.sh" "${PASSTHRU[@]}" --rom-id DMA_MODE2_HIDE
