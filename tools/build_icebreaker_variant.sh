...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help|-h)
            cat <<'EOF'
usage: tools/build_icebreaker_variant.sh --top <module> [--dry-run] [--skip-build] [--out-dir <dir>]
EOF
            exit 0
            ;;
        *)
...
[[ -n "${TOP_MODULE}" ]] || iceboy_die "--top is required"
if [[ -z "${OUT_DIR}" ]]; then
    OUT_DIR="${ICEBOY_ROOT}/build/variants/${TOP_MODULE}"
fi
NETLIST_JSON="${OUT_DIR}/${TOP_MODULE}.json"
...
iceboy_require_file "${NETLIST_JSON}" "synthesized variant netlist"
iceboy_require_file "${STAT_FILE}" "Yosys resource report"

echo "Built ${TOP_MODULE} from shared swim output."
