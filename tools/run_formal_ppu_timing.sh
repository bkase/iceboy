...
iceboy_require_file "${ICEBOY_ROOT}/formal/ppu/safety/ppu_timing.sby" "ppu timing sby job"
iceboy_require_file "${ICEBOY_ROOT}/formal/ppu/safety/ppu_timing_formal.sv" "ppu timing formal harness"

"${SWIM_BIN}" build >/dev/null
"${SBY_BIN}" --yosys "${YOSYS_BIN}" -f -d "${ICEBOY_ROOT}/build/formal/ppu_timing" "${ICEBOY_ROOT}/formal/ppu/safety/ppu_timing.sby"
