...
DRY_RUN=0
SKIP_BUILD=0
OUT_DIR="${ICEBOY_ROOT}/build/spram_synth_smoke"
SPADE_SV="${ICEBOY_ROOT}/build/spade.sv"
WRAPPER_SV="${ICEBOY_ROOT}/tools/verilog/wram_spram_synth_smoke_top.v"
TOP_MODULE="wram_spram_synth_smoke_top"
NETLIST_JSON=""
STAT_FILE=""
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help|-h)
            cat <<'EOF'
usage: tools/run_spram_synth_smoke.sh [--dry-run] [--skip-build] [--out-dir <dir>]
EOF
            exit 0
            ;;
        *)
...
NETLIST_JSON="${OUT_DIR}/wram_spram_test_top.json"
...
iceboy_require_file "${WRAPPER_SV}" "SPRAM smoke wrapper"
...
if ! rg -n "SB_SPRAM256KA|ICESTORM_SPRAM" "${STAT_FILE}" "${NETLIST_JSON}" >/dev/null; then
    iceboy_die "expected SPRAM primitive usage in ${STAT_FILE} or ${NETLIST_JSON}"
fi

echo "Verified SPRAM primitive usage for ${TOP_MODULE}."
