...
if [[ "${#ROM_IDS[@]}" -eq 0 ]]; then
    ...
    if [[ "${ICEBOY_PPU_WAVE_C_INCLUDE_RED:-0}" == "1" ]]; then
        ROM_IDS+=(OBJ_BASIC OBJ_PRIORITY OBJ_8X16 OBJ_FLIP OBJ_BG_MASK)
    fi
fi
...
if [[ "$DRY_RUN" == "1" ]]; then
    ...
    for rom_id in "${ROM_IDS[@]}"; do
        ...
        printf '\n'
        printf 'DRY RUN:'
        printf ' %q' "$RUNNER_BIN" "--rom=${rom_path}" "--frame-capture=${actual_raw}" "--trace=${trace_path}" "--max-mcycles=${MAX_MCYCLES}" "--stable-frames=${STABLE_FRAMES}" "--completed-frames=${COMPLETED_FRAMES}" "--progress-interval=${PROGRESS_INTERVAL}"
        printf '\n'
        printf 'DRY RUN:'
