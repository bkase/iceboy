...
class IcebreakerVisibleTopAssetsTest(unittest.TestCase):
    def test_visible_top_source_and_variant_helpers_are_checked_in(self) -> None:
        ...
        self.assertIn("inst framebuffer_spram(clk, false, scanout_i, lcd.pixel_advance)", top_text)
        self.assertIn("inst st7789_lcd(", top_text)
        self.assertIn("set DEBUG_GPIO0 = false;", top_text)
        self.assertIn("set DEBUG_GPIO1 = false;", top_text)
        self.assertIn("set DBG_PC0 = false;", top_text)
        self.assertIn("set DBG_MCE = false;", top_text)
        self.assertIn("set DBG_PHASE0 = false;", top_text)
