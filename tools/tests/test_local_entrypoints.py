...
class LocalEntrypointsTest(unittest.TestCase):
    ...
    def test_verify_hw_build_dry_run_uses_debug_free_contract(self) -> None:
        ...
        self.assertIn("reachable lib::sim imports", completed.stdout)
    ...
    def test_framebuffer_synth_probe_dry_run_targets_single_spram_top(self) -> None:
        completed = self.run_script("run_framebuffer_synth_probe.sh", "--dry-run", "--skip-build")
        self.assertEqual(completed.returncode, 0, completed.stderr)
        self.assertIn("[tool] uv: uv 0.0-test", completed.stdout)
        self.assertIn("[tool] swim: swim v0.17.0-test", completed.stdout)
        self.assertIn("[tool] yosys: Yosys 0.63+188", completed.stdout)
        self.assertIn("skip swim build", completed.stdout)
        self.assertIn("top video::framebuffer_probe_top::framebuffer_probe_top", completed.stdout)
        self.assertIn("module framebuffer_probe_top", completed.stdout)
        self.assertIn("src/video/framebuffer_probe_top.spade", completed.stdout)
        self.assertIn("build/hw_probes/framebuffer/hardware.json", completed.stdout)
        self.assertIn("build/hw_probes/framebuffer/yosys-stat.txt", completed.stdout)
        self.assertIn("synth_ice40 -top framebuffer_probe_top", completed.stdout)
        self.assertIn("SB_SPRAM256KA", completed.stdout)
        self.assertIn("SB_RAM40_4K", completed.stdout)

    def test_framebuffer_spram_synth_smoke_dry_run_targets_single_spram_module(self) -> None:
        completed = self.run_script("run_framebuffer_spram_synth_smoke.sh", "--dry-run", "--skip-build")
        self.assertEqual(completed.returncode, 0, completed.stderr)
        self.assertIn("[tool] uv: uv 0.0-test", completed.stdout)
        self.assertIn("[tool] swim: swim v0.17.0-test", completed.stdout)
        self.assertIn("[tool] yosys: Yosys 0.63+188", completed.stdout)
        self.assertIn("skip swim build", completed.stdout)
        self.assertIn("top video::framebuffer_spram_synth_top::framebuffer_spram_synth_top", completed.stdout)
        self.assertIn("module framebuffer_spram_synth_top", completed.stdout)
        self.assertIn("src/video/framebuffer_spram_synth_top.spade", completed.stdout)
        self.assertIn("build/hw_probes/framebuffer_spram/hardware.json", completed.stdout)
        self.assertIn("build/hw_probes/framebuffer_spram/yosys-stat.txt", completed.stdout)
        self.assertIn("synth_ice40 -top framebuffer_spram_synth_top", completed.stdout)
        self.assertIn("SB_SPRAM256KA", completed.stdout)
        self.assertIn("SB_RAM40_4K", completed.stdout)

    def test_uart_rx_synth_smoke_dry_run_targets_budgeted_uart_top(self) -> None:
        completed = self.run_script("run_uart_rx_synth_smoke.sh", "--dry-run", "--skip-build")
        self.assertEqual(completed.returncode, 0, completed.stderr)
        self.assertIn("[tool] uv: uv 0.0-test", completed.stdout)
        self.assertIn("[tool] swim: swim v0.17.0-test", completed.stdout)
        self.assertIn("[tool] yosys: Yosys 0.63+188", completed.stdout)
        self.assertIn("skip swim build", completed.stdout)
        self.assertIn("top periph::uart_rx_synth_top::uart_rx_synth_top", completed.stdout)
        self.assertIn("module uart_rx_synth_top", completed.stdout)
        self.assertIn("src/periph/uart_rx_synth_top.spade", completed.stdout)
        self.assertIn("build/hw_probes/uart_rx/hardware.json", completed.stdout)
        self.assertIn("build/hw_probes/uart_rx/yosys-stat.txt", completed.stdout)
        self.assertIn("synth_ice40 -top uart_rx_synth_top", completed.stdout)
    ...
    def test_hardware_soc_core_verilator_wrapper_dry_run_uses_bg_static_oracle_runner(self) -> None:
        ...
        self.assertIn("bench/roms/out/BG_STATIC.sym", completed.stdout)
        self.assertIn("__checkpoint_scene_ready", completed.stdout)
        self.assertIn("--rom-id=bg_static", completed.stdout)
        self.assertIn("--expected-raw=", completed.stdout)
        self.assertIn("skip swim build", completed.stdout)

    def test_hardware_soc_core_joypad_verilator_wrapper_dry_run_uses_scripted_oracle_runner(self) -> None:
        completed = self.run_script("run_hardware_soc_core_joypad_bg_smoke_verilator.sh", "--dry-run", "--skip-build")
        self.assertEqual(completed.returncode, 0, completed.stderr)
        self.assertIn("[tool] uv: uv 0.0-test", completed.stdout)
        self.assertIn("[tool] swim: swim v0.17.0-test", completed.stdout)
        self.assertIn("[tool] verilator: Verilator 5.046", completed.stdout)
        ...
        self.assertIn("--rom-id=joypad_bg_smoke", completed.stdout)
    ...
    def test_default_precommit_moves_long_rom_runs_to_extended_lane(self) -> None:
        ...
        self.assertIn("test/unit/test_hram_ebr.py", default_items)
        self.assertIn("test/unit/test_regs_bg_window.py", default_items)
        self.assertIn("test/ppu/unit/test_line_summary.py", default_items)
        self.assertIn("test/ppu/unit/test_oam_dma_mode2.py", default_items)
        ...
        self.assertNotIn('"test/unit/test_alu.py"', default_body)
        self.assertNotIn('"test/unit/test_st7789_lcd.py"', default_body)
        self.assertNotIn('"test/unit/test_framebuffer_spram.py"', default_body)
        self.assertNotIn('"test/unit/test_hw_backend.py"', default_body)
        self.assertNotIn('"test/unit/test_vram_ebr.py"', default_body)
        ...
        self.assertNotIn('"test/ppu/unit/test_bg_transfer_live.py"', default_body)
        self.assertNotIn('"test/ppu/unit/test_bg_fifo.py"', default_body)
        ...
        self.assertIn("test/unit/test_alu.py", extended_items)
        self.assertIn("test/unit/test_st7789_lcd.py", extended_items)
        self.assertIn("test/unit/test_framebuffer_spram.py", extended_items)
        self.assertIn("test/unit/test_hw_backend.py", extended_items)
        self.assertIn("test/unit/test_vram_ebr.py", extended_items)
        self.assertIn("tools/run_cpu_instrs_blargg_verilator.sh", extended_items)
        self.assertIn("tools/run_icebreaker_alu_loop_verilator.sh", extended_items)
        self.assertIn("tools/run_joypad_bg_smoke_verilator.sh", extended_items)
        self.assertIn("tools/run_ppu_checker_ball_verilator.sh", extended_items)
        self.assertIn("tools/run_dma_mode2_hide_verilator.sh", extended_items)
        self.assertIn("tools/run_obj_dma_metadata_corrupt_verilator.sh", extended_items)
        self.assertIn("test/rom/test_loads_basic.py", extended_items)
        self.assertIn("test/ppu/unit/test_ppu_core_smoke.py", extended_items)
        self.assertIn("test/ppu/unit/test_bg_transfer_live.py", extended_items)
        self.assertIn("test/ppu/unit/test_bg_fifo.py", extended_items)
        self.assertIn("test/ppu/unit/test_mixer.py", extended_items)
        self.assertIn("test/ppu/unit/test_obj_observe.py", extended_items)
        self.assertIn("test/ppu/unit/test_obj_priority.py", extended_items)
        self.assertIn("test/ppu/unit/test_obj_fetch.py", extended_items)
        self.assertIn("test/ppu/unit/test_transfer_penalty.py", extended_items)
        self.assertIn("test/ppu/unit/test_obj_transfer_live.py", extended_items)
        self.assertIn("test/ppu/unit/test_window.py", extended_items)
        self.assertIn("test/ppu/unit/test_tile.py", extended_items)
        self.assertIn("test/power/test_ppu_power_quiescence.py", extended_items)
    ...
    def test_precommit_skips_redundant_heavy_python_modules_and_formal_by_default(self) -> None:
        ...
        self.assertIn('tools/run_ppu_wave_b_mealybug_verilator.sh', text)
