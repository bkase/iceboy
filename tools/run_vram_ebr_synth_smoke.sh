...
DRY_RUN=0
SKIP_BUILD=0
OUT_DIR="${ICEBOY_ROOT}/build/vram_ebr_synth_smoke"
SPADE_SV="${ICEBOY_ROOT}/build/spade.sv"
WRAPPER_SV="${ICEBOY_ROOT}/tools/verilog/vram_ebr_synth_smoke_top.v"
TOP_MODULE="vram_ebr_synth_smoke_top"
NETLIST_JSON=""
STAT_FILE=""
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help|-h)
            cat <<'EOF'
usage: tools/run_vram_ebr_synth_smoke.sh [--dry-run] [--skip-build] [--out-dir <dir>]
EOF
            exit 0
            ;;
        *)
...
NETLIST_JSON="${OUT_DIR}/vram_ebr_synth_test_top.json"
...
iceboy_require_file "${WRAPPER_SV}" "VRAM EBR smoke wrapper"
...
if (( EBR_COUNT != 16 )); then
    iceboy_die "expected VRAM synth smoke to use exactly 16 SB_RAM40_4K blocks, got ${EBR_COUNT}"
fi

echo "Verified ${TOP_MODULE} maps to 16 SB_RAM40_4K blocks."
